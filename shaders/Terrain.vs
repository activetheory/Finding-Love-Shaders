...
float aastep(float threshold, float value, float buf) {
   return smoothstep(threshold - buf, threshold + buf, value);
}
...
void main() {
    float noise = range(snoise((position * 0.054) + time), -1.0, 1.0, 0.0, 1.0);
    noise = 1.0 - aastep(0.3, noise, 0.4);

    vAlpha = transitionMask(uv);

    float posLen = length(position);
    float height = range(posLen, 5.0, 30.0, 0.2, 20.0);

    float raise = height * noise;
    raise *= crange(posLen, 5.0, 10.0, 0.2, 1.0);

    vec3 pos = position;
    pos.y += raise * heightMask(uv);

    vSat = crange(pos.y, 0.0, 6.0, 0.0, 0.1);

    vec4 mvPosition = modelViewMatrix * vec4(pos, 1.0);
    vViewPosition = -mvPosition.xyz;
    vWorldPos = (vec4(pos, 1.0) * modelMatrix).xyz;
    vPos = pos;
    vUv = uv;
